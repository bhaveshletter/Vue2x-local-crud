...
Vue.component('user', {
  ...
  template: '<div>\
   ...
   <ul>\
    <li v-for="item in result" v-bind:key="item.id" @click="fullInfo(item)" class="pointer">\
      {{item.username}},\
      {{item.email}}\
    </li>\
   </ul>\
...
Vue.component('album', {
  ...
  created: function(){
    this.getData('https://jsonplaceholder.typicode.com/albums')
  }
})
...
Vue.component('movie-info', {
  ...
  template: '<li>\
              ...
              <b> Name:</b> {{movie.name}},\
              <b> Type:</b> {{movie.type}}\
              <modal v-if="isPopupOpen">\
                <h3 slot="header">{{movie.name}}\'s detail</h3>\
                <h3 slot="body">\
                {{movie}}\
                </h3>\
                <h3 slot="footer">\
                  <button class="modal-default-button" @click="isPopupOpen = false">\
                    Ok\
                  </button>\
                </h3>\
              </modal>\
              <button id="show-modal" @click="isPopupOpen = true">Detail</button>\
            </li>'
})
